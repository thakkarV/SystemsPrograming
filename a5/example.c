...
/* List a source lines in the given DIE.
 */
void list_src_lines_in_die(Dwarf_Debug dbg, Dwarf_Die the_die)
{
	int i;
	Dwarf_Error error;
	Dwarf_Signed cnt;
	Dwarf_Line *linebuf;
	if(dwarf_srclines(the_die, &linebuf, &cnt, &error) == DW_DLV_OK) {
		for(i = 0; i < cnt; ++i) {
...
void list_funcs_in_file(Dwarf_Debug dbg)
{
	...
	list_src_lines_in_die(dbg, cu_die);
